...
describe("Learna", function () {
  ...
  describe("Should record weekly payout", function () {
    it("An admin should be able to sort weekly earning payroll", async function () {
        const { learna, knowTokenAddr, signers : { deployer, signer1Addr },} = await loadFixture(deployContractsFixcture);
        ...
        const pf = await recordPoints({deployer, learna, quizResult, campaignHash, user: signer1Addr, token: knowTokenAddr});
