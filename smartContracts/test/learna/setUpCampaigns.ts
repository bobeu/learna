...
describe("Learna", function () {
  ...
  describe("Setup campaigns", function () { 
    it("Should setup and fund campaign successfully", async function () {
      const { learna, knowToken, learnaAddr, signers : { signer1, signer1Addr, deployer }, knowTokenAddr} = await loadFixture(deployContractsFixcture);
      ...
      const {
        ...
        campaigns: cp
      } = await setUpCampaign({learna, signer: signer1, campaign: campaigns[0], fundERC20, token: knowToken, value});
      ...
      expect(cp.campaigns[0].data.token).to.be.eq(knowTokenAddr);
