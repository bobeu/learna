...
import { ChevronDown, ChevronUp, BarChart3, Layers, Calendar, ImageIcon, WandSparkles, Loader2, ArrowLeft, PlusCircle } from "lucide-react";
...
const ButtonTemplate = ({isDark, isExit, disabled, text, onClick} : {isDark: boolean; isExit: boolean; disabled: boolean; text?: string; onClick: () => void}) => {
    return(
         <Button 
            disabled={disabled}
            variant="outline" 
            className={`gap-2 ${isDark ? 'text-white border-neutral-700' : ''}`}
            onClick={onClick}
        >
            {
                isExit? <ArrowLeft className="w-4 h-4" /> : <PlusCircle className="w-4 h-4" />
            }
            {`${text || 'New Campaign'}`}
        </Button>
    )
...
export default function NewCampaignPage(){
    ...
    const selectedCampaign = React.useMemo(() => {
        let selectedCampaign = formattedMockCampaignsTemplate[0];
        if(myCampaigns && myCampaigns.length > 0 && selectedIndex !== null){
            selectedCampaign = myCampaigns[selectedIndex]._raw;
        }
        return selectedCampaign;
    }, [selectedIndex, myCampaigns]);
                                ...
                                {showMyCampaigns && (
                                    <div className="space-y-2">
                                        {myCampaigns.map((c, index) => (
                                            <button key={c.id} 
                                                className={`w-full text-left p-3 rounded-lg border border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-900 ${c._raw.metadata.name === selectedCampaign?.metadata.name ? 'bg-neutral-100 dark:bg-neutral-900' : ''}`} 
                                                onClick={() => {
